...
return_activity = True, animation = False, anim_name = None, from_file = False, save_file=False):
    '''
    ...
    #nest.SetKernelStatus({'local_num_threads' : num_omp})
    ...
    if w_IC is not None:
        ...
        if type(w_IC) is tuple:
            ns.randomize_neural_states(net, instructions = {'w' : w_IC})
        else:
            nest.SetStatus(gids,'w',w_IC,_warn=False)
            net.set_node_attribute('w', values=w_IC, value_type="double")
    
    if Vm_IC is not None:
        
        if type(Vm_IC) is tuple:
            ns.randomize_neural_states(net, instructions = {'V_m' : Vm_IC})
        else:
            nest.SetStatus(gids,'V_m',Vm_IC,_warn=False)
            net.set_node_attribute('V_m', values=Vm_IC, value_type="double")
    ...
    recorder_vm, record_vm = nngt.simulation.monitor_groups(group_names,net,nest_recorder="multimeter",params={"withtime":True, "record_from":["V_m","w"] })
    ...
    Vms      = nest.GetStatus(recorder_vm[0])[0]["events"]["V_m"]
    time_vms = nest.GetStatus(recorder_vm[0])[0]["events"]["times"]
    w        = nest.GetStatus(recorder_vm[0])[0]["events"]["w"]
    ...
    if return_activity:
        return senders ,times, Vms, time_vms, w
...
if __name__ == '__main__':
    ...
    w = 20.		
    # Make the network
    net = Make_Network(net_file, num_omp = 4, culture_radius = 1000, num_neurons = 500, avg_deg = 50, lambda_edr = 150.,
    delay_slope = 0.01, delay_offset = 2., syn_weight = w, )
    # Simulate
    senders , spikes = Simulation(net_file, 4, sim_time = 15000, noise_rate = 10., noise_weight_fraction = 0.5, 
